    ...
    attach: function (context, settings) {
      ...
      $('.brk-tabs:not(.rendered)', context).addClass('rendered').each(function () {
        if (!$(this).hasClass('brk-tabs-hovers')) {
          ...
          var fnlogic_tab_hover = false;

          var tabHover = function (widthDoc) {
            var windowWidth = window.innerWidth || $(window).width();

            if (windowWidth < widthDoc && !fnlogic_tab_hover) {

              tab.unbind('mouseenter').on('click', function () {
                var $this = $(this),
              ...
              /* ------------------- fnlogic_tab_hover_flexmenu ---------------------*/
              fnlogic_tab_hover = true;
            } else if (windowWidth >= widthDoc && fnlogic_tab_hover) {

              tab.unbind('click').on('mouseenter', function () {
                var $this = $(this),
                  $icon = $this.children('[class*="fa-"]');
